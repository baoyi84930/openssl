...
The TLS EndOfEarlyData message is not used with QUIC. when SSL_OP_NO_END_OF_EARLY_DATA
is set, The server will enable 0-RTT, and skip the EndOfEarlyData message. Currently
only affects for server.
